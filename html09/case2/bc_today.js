...
//Grabs todays date (LONG FORMAT and TIME ZONE)
const thisDate = new Date();

//Converts thisdate long date into short date (7/27/2021)
const dateString = thisDate.toLocaleDateString();

//wraps the converted dateString into h2 heading for html inject and stores
const dateHTML = `<h2>${dateString}</h2>`;

//converts the day of week into number variable from 0-6 using the long date originally grabbed
const thisDay = thisDate.getDay();

//runs the getevent function using the day of week number and stores into a new variable
const eventHTML = getEvent(thisDay);

//searches html for element id then inserts before end the the 2 newly stored html variables
document
  .getElementById("unionToday")
  .insertAdjacentHTML("beforeend", dateHTML + eventHTML);

//Provided event by day of week SUN - SAT
